...
public class ReceptorHogaresTest {
    ...
    RespuestaDeHogares fakeResponse = null;

    @BeforeEach
    public void context() throws JsonProcessingException {
        fakeResponse = objectMapper.readValue(testJson, RespuestaDeHogares.class);
    }

    @Test
    public void obtenerHogaresRetornaUnaListaDeHogaresConUnSoloElemento() throws JsonProcessingException {
        when(receptorHogaresMock.getJsonMapeado("1")).thenReturn(fakeResponse);
        RespuestaDeHogares hogarResponse = receptorHogaresMock.getJsonMapeado("1");
    ...
    public void obtenerTodosLosHogaresDevuelveUnaListaCon5Elementos() throws JsonProcessingException {
        ReceptorHogares hogarClient = new ReceptorHogares();
        ReceptorHogares spyClient = Mockito.spy(hogarClient);
        //Se hace 4 veces porque por ahora tenemos harcodeado en el client que se haga un for 4 veces. HAbria que cambiarlo.
        Mockito.doReturn(fakeResponse).when(spyClient).getJsonMapeado(Mockito.any());
        Mockito.doReturn(fakeResponse).when(spyClient).getJsonMapeado(Mockito.any());
        Mockito.doReturn(fakeResponse).when(spyClient).getJsonMapeado(Mockito.any());
        Mockito.doReturn(fakeResponse).when(spyClient).getJsonMapeado(Mockito.any());
        List<RespuestaDeHogar> resultado = spyClient.obtenerTodosLosHogares();
